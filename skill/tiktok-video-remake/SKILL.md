...
- B1 宫格分镜必须使用批准后的目标商品，不得残留源片商品、配件或包装。
...
3. 使用可用的图像编辑/生成能力只生成一张完整 B1 宫格；一次生成完整宫格，不逐格生成或拼接独立最终分镜，不生成 A1、A2 或 B2。
4. B1 忠实执行所选 BASE/Vxx 的语义与时间线，同时强化前 3 秒主体可见性、商品进入前景的大小对比、关键动作瞬间、景别落差、动作轴和构图方向；不得新增剧情、卖点、商品事实或不可连续的动作，也不得让视觉强化改变批准脚本。
5. B1 中所有商品相关对象都必须换成批准 SKU；人物、场景、格数、编号、关键状态、时间线语义和非商品道具必须与有效脚本一致。
6. 保存到 `02-storyboard/<BASE-or-Vxx>/storyboard-grid-b1.png`。宫格编号放在格子边框或留白中，不覆盖主体。
7. 默认不运行 `scripts/crop_storyboard_grid.py`，不生成单格裁剪图。只有用户明确要求裁剪时，才从 B1 机械裁到 `02-storyboard/<BASE-or-Vxx>/crops-b1/`；裁剪不属于重新生成，也不加入默认交付。
8. 输出该分支唯一视频提示词 `03-symphony/<BASE-or-Vxx>/symphony-prompt.txt`。它必须是一段可整体复制的纯文本，对应一次整条视频生成。
9. 在提示词中增加简洁的“人物外观锚点”和“环境视觉锚点”，只写跨镜头稳定、且与 B1 不冲突的身份与环境特征；不要在锚点中写精确站位、临时姿态、瞬时表情、具体景别或主体画面占比。宫格存在时由 B1 锁定这些动态构图细节，没有宫格时由完整时间线锁定。
...
13. 首次直接展示 B1 宫格和 `symphony-prompt.txt`，不增加选择步骤。说明 B1 用于增强构图稳定性，也可只使用提示词配合产品图；若不满意，可回复“B1 + 第几格 + 具体问题”，随后只重做并替换整张 B1，保持其他格与时间线稳定。仅视觉修正不重写 Symphony 提示词；只有反馈改变人物稳定特征、环境稳定特征、镜头语义、时间码、台词或商品事实时才同步更新提示词。
